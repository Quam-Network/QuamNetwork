...
contract PrivateSale is Ownable {
    ...
    uint256 public tokenPrice = 15e15; //in usd: 0.015
    ...
    function buyTokenWithBUSD(uint256 _busdAmount) external onlyWhitelisted {
        ...
        if (
            whitelist[msg.sender].alloc > 0 && whitelist[msg.sender].bought == 0
        ) {
        ...
        //5% discount
        uint256 tokenAmount = _busdAmount.mul(1e18).div(tokenPrice);
        ...
        if (
            ...
            uint256 actualBUSDSpent = tokenAmount.mul(tokenPrice).div(1e18);
            busdReturn = _busdAmount.sub(actualBUSDSpent);
        }
        ...
        busd.safeTransfer(tokenOwner, busd.balanceOf(address(this)));
    }
    ...
    function payTokenForRange(uint256 _from, uint256 _to) external {
        uint256 length = whitelistList.length;
        _to = _to < length - 1 ? _to : length - 1;
        for (uint256 i = _from; i <= _to; i++) {
            if (whitelist[whitelistList[i]].bought > 0) {
                uint256 _amount = whitelist[whitelistList[i]]
                    .bought
                    .mul(20)
                    .div(100);
                totalUnlocked = totalUnlocked.add(_amount);
                quam.safeTransferFrom(msg.sender, whitelistList[i], _amount);
            }
        }
    }
